...
const MyInfoCertification = () => {
  ...
  return (
    <div className={cx("CertificationWrapper")}>
      ...
      <p>정보를 안전하게 보호하기 위해 보인인 실명과 핸드폰 뒷자리 번호를 입력해주세요.</p>
      ...
      <div className={cx("AbleBtnWrapper")}>
        <Link href="/mypage/myinfo">{isButtonDisabled ? <DisableBtn label={"확인"} /> : <AbleBtn label={"확인"} onClick={handleConfirmation} />}</Link>
      </div>
    </div>
  );
};
