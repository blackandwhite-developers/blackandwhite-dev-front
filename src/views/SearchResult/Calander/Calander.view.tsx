...
const CalanderView = () => {
  ...
  const handleDateChange = (date: DateRange | undefined) => {
    ...
    if (date && date.from && date.to) {
      ...
      const formattedDateRange = `${formatDate(date.from)} ~ ${formatDate(date.to)}`;
  ...
  return (
    <div className={cx("calander-container")}>
      ...
      <div className={cx("about")}>
        ...
        <MemberBtn label={<>{`성인 ${adultCount}명\n아동 ${childCount}명`}</>} />
      </div>

      <div className={cx("calander")}>
        <DayPicker locale={ko} mode="range" selected={selectedDateRange} onSelect={handleDateChange} />
      </div>
