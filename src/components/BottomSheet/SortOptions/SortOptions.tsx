...
const SortOptionList = ({ sortOptions, onClose }: { sortOptions: string[]; onClose: () => void }) => {
        ...
        {sortOptions.map((sortOption, index) => (
          <div
            className={cx("CategoryList", {
              selectedSortOption: selectedSortOption === sortOption,
            })}
            key={index}
            onClick={() => handleSortOptionClick(sortOption)}
          >
            <p className={cx("CategoryItem")}>{sortOption}</p>
            {selectedSortOption === sortOption && <FaCheck className={cx("CategoryItemCheck")} />}
          </div>
        ))}
      </div>
    </div>
